...
public class Trans2PinYin {
	...
	private static int[] icPinYinValue = new int[] { -20319, -20317, -20304, -20295,
	...
	private static String[] scPinYin = new String[] { "a", "ai", "an", "ang",
	...
	static private int getChsAscii(String chs) {
		int asc = 0;
		try {
	...
	static public char OneCharToHeadPinYin(String sText) {
		char cPinYin = 0;
		int iAsc = getChsAscii(sText);
		if (iAsc > 0 && iAsc < 160) {
			cPinYin = (char) iAsc;
		} else {
			for (int i = (icPinYinValue.length - 1); i >= 0; i--) {
				if (icPinYinValue[i] <= iAsc) {
					cPinYin = scPinYin[i].charAt(0);
					break;
				}
			}
		}
	...
	static public String OneCharToFullPinYin(String sText) {
		String sPinYin = null;
		int iAsc = getChsAscii(sText);
		if (iAsc > 0 && iAsc < 160) {
			sPinYin = String.valueOf((char) iAsc);
		} else {
			for (int i = (icPinYinValue.length - 1); i >= 0; i--) {
				if (icPinYinValue[i] <= iAsc) {
					sPinYin = scPinYin[i];
	...
	static public String StringToHeadPinYin(String sText) {
		StringBuilder sb = new StringBuilder();
		for (int j = 0; j < sText.length(); j++) {
			char c = OneCharToHeadPinYin(sText.substring(j, j + 1));
			if (c > 0) {
				sb.append(c);
			}
		}
	...
	static public String StringToFullPinYin(String sText) {
		StringBuilder sb = new StringBuilder();
		for (int j = 0; j < sText.length(); j++) {
			String oneChar = OneCharToFullPinYin(sText.substring(j, j + 1));
			if (oneChar != null) {
				sb.append(oneChar);
			}
		}
	...
	public static void main(String[] args) {
		String str = "你好，Hello！";
		Log.log(str);
		Log.log(StringToFullPinYin(str));
		Log.log(StringToHeadPinYin(str));
